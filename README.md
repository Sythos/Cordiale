...
- **Watch Lists**: the presence (notify) list shown for the network
  ...
  `presence_snapshot` (online, offline, or plain when unknown). The keyword
...
- **Realtime event coverage (0.1.5 ALPHA (WIP))**: the current Grappa protocol
  lists 56 top-level event kinds. Cordiale handles 49 of them:
  `links_bundle`,
  ...
  `notify_list`, `presence_snapshot`, and `message`; the remaining 7 kinds
  are
  ...
  - **Presence, queries, and server replies**: `presence_changed`,
    `presence_error`, `mentions_bundle`.
